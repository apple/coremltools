...
class TestUpsampleBilinear:
    def test_builder_to_backend_smoke(self, use_cpu_only=True, backend='nn_proto'):
        ...
        def build_upsample_integer(x):
            return mb.upsample_bilinear(x=x,
...
class TestUpsampleNearestNeighbor:
    def test_builder_to_backend_smoke(self, use_cpu_only=True, backend='nn_proto'):
        ...
        def build(x):
            return mb.upsample_nearest_neighbor(x=x,
...
class TestCropResize:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend, is_symbolic):
        ...
        expected_output_type = [(N, placeholder_input_shape[0], placeholder_input_shape[1], 2, 2, types.fp32),
                                (N, placeholder_input_shape[0], placeholder_input_shape[1], 4, 4, types.fp32),
                                (N, placeholder_input_shape[0], placeholder_input_shape[1], 1, 1, types.fp32),
                                (N, placeholder_input_shape[0], placeholder_input_shape[1], 2, 2, types.fp32),
                                (N, placeholder_input_shape[0], placeholder_input_shape[1], 2, 2, types.fp32)
        ]
