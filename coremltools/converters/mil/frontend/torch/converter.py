...
class TranscriptionContext:
    ...
    def add(self, ssa_var: Var, torch_name: Optional[str] = None) -> None:
        """
        Arguments:
...
class TorchConverter:
    ...
    def __init__(
        ...
        if isinstance(loaded_model, torch.jit.ScriptModule):
            self.graph, self.params_dict = InternalTorchIRGraph.from_torchscript(
            ...
            self.params_dict = (
                None  # TODO(gdawer): On Torchscript, this is needed for Training models
            )
        else:
            raise ValueError(
