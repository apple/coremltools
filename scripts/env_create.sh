...
if [[ $include_build_deps == 1 ]]; then
    # The no-binary flag is needed due to https://github.com/protocolbuffers/protobuf/issues/3002.
    python -m pip install --prefer-binary --no-binary=protobuf -r $COREMLTOOLS_HOME/reqs/build.pip
fi
...
if [[ $include_test_deps == 1 ]]; then
  ...
  python -m pip install --prefer-binary --no-binary=protobuf -r $COREMLTOOLS_HOME/reqs/test.pip --upgrade
fi
...
if [[ $include_docs_deps == 1 ]]; then
  ...
  python -m pip install --prefer-binary --no-binary=protobuf -r $COREMLTOOLS_HOME/reqs/docs.pip --upgrade
fi
