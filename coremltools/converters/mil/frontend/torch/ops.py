        ...
        for axis_x, axis_y in zip(axes_x, axes_y):
            ...
            if is_symbolic(size_x) or is_symbolic(size_y):
                raise ValueError(
                    ...
                    f"dimension {axis_x} of self and {axis_y} of other"
                )
            if size_x != size_y:
                raise ValueError(
                    f"tensordot contracts dimension {axis_x} of self with dimension "
                    f"{axis_y} of other, so they must match, but got {size_x} and {size_y}"
                )
            volume *= size_x
        return volume
