...
from coremltools.converters.nnv2.frontend.torch.torch_op_registry import register_torch_op
from coremltools.converters.nnv2.frontend.torch.torch_op_registry import _TORCH_OPS_REGISTRY as _TORCH_OPS_REG
from coremltools.converters.nnv2.frontend.torch.ops import _get_inputs
...
def cosine_similarity(context, node):
    ...
    dim = inputs[-2].val
    eps = inputs[-1].val
    xy = cb.mul(x=inputs[0], y=inputs[1])
    sum_xy = cb.reduce_sum(x=xy, axes=[dim])

    xx = cb.mul(x=inputs[0], y=inputs[0])
    sum_xx = cb.reduce_sum(x=xx, axes=[dim])
    yy = cb.mul(x=inputs[1], y=inputs[1])
    sum_yy = cb.reduce_sum(x=yy, axes=[dim])

    mul_sum_xy = cb.mul(x=sum_xx, y=sum_yy)
    div_12 = cb.maximum(x=mul_sum_xy, y=eps*eps)
    div_sqrt = cb.sqrt(x=div_12)

    cs = cb.real_div(x=sum_xy, y=div_sqrt, name=node.name)
    context.add(cs)
...
class TestCustomOp:
    ...
    def _sparse_mm(context, node):
        ...
        x = cb.custom_torch_sparse_matmul(x=inputs[0], y=inputs[1],
                             x_is_sparse=True, y_is_sparse=True,
                             name=node.name)
        context.add(x)
    ...
    def test_custom_sparse_mm_op(self, input_shape=(4, 4)):
        ...
        assert False == layers[-1].custom.parameters['transpose_x'].boolValue, "Incorrect parameter value k"
        assert False == layers[-1].custom.parameters['transpose_y'].boolValue, "Incorrect parameter value k"
        assert True == layers[-1].custom.parameters['x_is_sparse'].boolValue, "Incorrect parameter value k"
        assert True == layers[-1].custom.parameters['y_is_sparse'].boolValue, "Incorrect parameter value k"