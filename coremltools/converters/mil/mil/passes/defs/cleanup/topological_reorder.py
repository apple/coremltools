...
class topological_reorder(AbstractGraphPass):
    """
    Topologically re-orders the list of operations in a program by places each operation closer to its
    first use, or at the end if it's not consumed by any other operation.
        ...
        for op in reversed(block.operations):
            ...
            if op.op_type in op_type_to_move:
                ...
                if len(first_consumers) > 0:
                    ...
                    first_use_indices = [
                        block.operations.index(first_use_op) for first_use_op in first_consumers
                    ]
            ...
            for b in current_op.blocks:
                relevant_inputs |= topological_reorder._move_operations_to_the_end_block(
                    b, op_type_to_move
                )
            ...
            for v in relevant_inputs:
                ...
                if v not in first_use or block.operations.index(
                    first_use[v]
                ) > block.operations.index(current_op):
