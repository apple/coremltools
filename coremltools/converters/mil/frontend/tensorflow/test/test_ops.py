...
class TestAddN:
    @pytest.mark.parametrize("use_cpu_only, backend, rank, num_inputs",
                             itertools.product(
                                 [True, False],
                                 backends,
                                 list(range(6)),
...
class TestSelect:
    ...
    def test_select(self, use_cpu_only, backend, rank, broadcast, dynamic):
        ...
        run_compare_tf(model, inputs_dic, outputs,
...
class TestCond:
    ...
    def test_cond(self, use_cpu_only, backend):
        ...
        model, inputs, outputs = build_model
        input_values = [np.array([1], dtype=np.float32),
    ...
    def test_cond_multi_returns(self, use_cpu_only, backend):
        ...
        def build_model(x, y):
            ...
            def true_fn(): return tf.add(x, z), tf.math.multiply(x, z)

            def false_fn(): return tf.square(y), tf.sqrt(z)
    ...
    def test_cond_nested_1(self, use_cpu_only, backend):
        ...
        def build_model(x, y):
            ...
            cond_1 = tf.cond(
                f, lambda: tf.pow(x, y), lambda: tf.math.subtract(x, y))
            cond_2 = tf.cond(
                t, lambda: tf.multiply(x, y), lambda: tf.math.mod(x, y))
            cond_3 = tf.cond(
                f, lambda: tf.math.divide(x, y), lambda: cond_2)
...
class TestWhileLoop:
    ...
    def test_while_loop_1(self, use_cpu_only, backend):
        ...
        def build_model(x, y):
            c = lambda i, j: \
                tf.greater(tf.math.reduce_mean(i), tf.math.reduce_mean(j))
    ...
    def test_while_loop_3(self, use_cpu_only, backend):
        ...
        def build_model(x, y, z):
            c = lambda i, j, k: tf.greater(tf.math.reduce_mean(i), tf.math.reduce_mean(j))
    ...
    def test_nested_while_body(self, use_cpu_only, backend):
        ...
        model, inputs, outputs = build_model
        input_values = [np.array([0], dtype=np.float32),
                        np.array([10], dtype=np.float32)]
        input_dict = dict(zip(inputs, input_values))
        run_compare_tf(model, input_dict, outputs,
                       use_cpu_only=use_cpu_only,
...
class TestConv:
    ...
    def test(
        ...
        if dynamic_weights and dilations == (1, 1):
            ...
            input_values = [
                random_gen(input_shape, -10., 10.), random_gen(W_shape, -1.0, 1.0)]
            input_dict = dict(zip(inputs, input_values))
...
class TestDepthwiseConv:
    @pytest.mark.parametrize(
        ...
        itertools.product(
            ...
            [(1, 1, 1, 1), (1, 2, 2, 1)],
            [(1, 1), (2, 2)],  # rdar://60668562 (MIL: Conversion for TF op 'SpaceToBatchND' not implemented.)
            [True, False],
            [1, 3]
        ))
    def test_depthwise_conv(
        ...
        def test_dynamic_W():
            ...
            run_compare_tf(model, input_dict, outputs, use_cpu_only=use_cpu_only,
                            backend=backend, frontend_only=False)
...
class TestSeparableConv:
    ...
    def test_separable_conv(
        ...
        def test_dynamic_W():
            @make_tf_graph([input_shape, depthwise_filter_shape, pointwise_filter_shape])
            def build_model_dynamic_weights(x, depthwise_filter, pointwise_filter):
                return tf.nn.separable_conv2d(x, depthwise_filter, pointwise_filter, strides=strides, padding=padding,
                                          dilations=dilations, data_format=data_format)
            model, inputs, outputs = build_model_dynamic_weights
        ...
        def test_static_W():
            depthwise_filter = (np.random.rand(*depthwise_filter_shape).astype(np.float32))
            pointwise_filter = (np.random.rand(*pointwise_filter_shape).astype(np.float32))
            @make_tf_graph([input_shape])
            def build_model_static_weights(x):
                return tf.nn.separable_conv2d(x, depthwise_filter, pointwise_filter, strides=strides, padding=padding,
                                          dilations=dilations, data_format=data_format)
...
class TestElementWiseBinary:
    ...
    def test_binary(self, use_cpu_only, backend, rank, mode):
        ...
        if mode == 'add':
            ...
            if backend == 'mil_proto': return  # TODO
    ...
    def test_equal(self, use_cpu_only, backend, rank):
        ...
        input_values = [np.random.randint(low=-5, high=3, size=x_shape).astype(dtype),
...
class TestImageResizing:
    ...
    def test_resize_bilinear(self, use_cpu_only, backend, input_shape, target_shape,
    ...
    def test_upsampling_2d(self, use_cpu_only, backend, input_shape, upsample_factor, data_format):
        if data_format == 'channels_last':
            input_shape = (input_shape[0], input_shape[2], input_shape[3], input_shape[1])
        with tf.Graph().as_default() as graph:
            x = tf.placeholder(tf.float32, shape=input_shape)
            ref = tf.keras.layers.UpSampling2D(
                size=upsample_factor, data_format=data_format, interpolation="nearest")(x)
            run_compare_tf(graph, {x: random_gen(input_shape, rand_min=-100, rand_max=100)},
                           ref, use_cpu_only=use_cpu_only, backend=backend)
...
class TestLinear:
    @pytest.mark.parametrize('use_cpu_only, backend, dim, transpose_a, transpose_b, use_constant',
                             itertools.product(
                                 ...
                                 [True, False],
                                 [True, False]))
    def test_matmul(self, use_cpu_only, backend, dim, transpose_a, transpose_b, use_constant):
        ...
        if not use_constant:
            ...
            def build_model(x, y):
                return tf.linalg.matmul(x, y, transpose_a=transpose_a, transpose_b=transpose_b)
            input_values = [random_gen(shape=shape_x, rand_min=-100, rand_max=100),
                            random_gen(shape=shape_y, rand_min=-1., rand_max=1.)]
        else:
            y = random_gen(shape=shape_y, rand_min=-1., rand_max=1.)
        ...
        proto = run_compare_tf(model, input_dict, outputs, use_cpu_only=use_cpu_only, backend=backend)
...
class TestNormalization:
    ...
    def test_batch_norm(self, use_cpu_only, backend, rank, shape_mode, epsilon):
        ...
        input_values = [random_gen(shape=input_shape, rand_min=-100., rand_max=100.),
                        random_gen(shape=attr_shape, rand_min=-1., rand_max=1.),
                        random_gen(shape=attr_shape, rand_min=0., rand_max=10.),
                        random_gen(shape=attr_shape, rand_min=1., rand_max=10.),
                        random_gen(shape=attr_shape, rand_min=-1., rand_max=1.)]
        input_dict = dict(zip(inputs, input_values))
    ...
    def test_batch_norm_with_global_normalization(
        ...
        if scale_after_normalization:
            ...
            def build_model(x, m, v, b, g):
                return tf.nn.batch_norm_with_global_normalization(
                    x, mean=m, variance=v, beta=b, gamma=g, variance_epsilon=epsilon,
                    scale_after_normalization=scale_after_normalization)
        else:
            @make_tf_graph([input_shape, attr_shape, attr_shape, attr_shape])
            def build_model(x, m, v, b):
                return tf.nn.batch_norm_with_global_normalization(
                    x, mean=m, variance=v, beta=b, gamma=None, variance_epsilon=epsilon,
                    scale_after_normalization=scale_after_normalization)
        ...
        input_values = [random_gen(shape=input_shape, rand_min=-100., rand_max=100.),
                        ...
                        random_gen(shape=attr_shape, rand_min=1., rand_max=10.)]
        if scale_after_normalization:
            input_values.append(random_gen(shape=attr_shape, rand_min=-1., rand_max=1.))
    ...
    def test_fused_batch_norm(self, use_cpu_only, backend, epsilon):
        ...
        @make_tf_graph([input_shape])
        def build_model(x):
            return tf.compat.v1.nn.fused_batch_norm(x, mean=m, variance=v, offset=o, scale=s,
    ...
    def test_local_response_normalization(self, use_cpu_only, backend, size, alpha, beta, k):
    ...
    @pytest.mark.parametrize('use_cpu_only, backend, rank_and_axes, keep_dims, tf_op',
                             itertools.product(
                                 ...
                                 [tf.reduce_all, tf.math.reduce_euclidean_norm,
                                  ...
                                  tf.reduce_prod, tf.reduce_sum, tf.reduce_any,
                                  tf.reduce_logsumexp,
...
class TestScatterGather:
    ...
    @pytest.mark.parametrize("use_cpu_only, backend, data_rank, indices_rank",
                             itertools.product(
                                 [True, False],
                                 backends,
                                 list(range(1,4)),
    ...
    def test_scatter_nd_with_zeros(self, use_cpu_only, backend, data_rank, indices_rank):
        ...
        @make_tf_graph([list(indices.shape)+[tf.int32], updates_shape, [data_rank, tf.int32]])
...
class TestSlice:
    @pytest.mark.parametrize('use_cpu_only, backend, rank, masking, trial',
                             itertools.product(
                                 [True, False],
                                 backends,
    ...
    def test_slice_by_index(self, use_cpu_only, backend, rank, masking, trial):
        input_shape = np.random.randint(low=2, high=6, size=rank)
        begin_val = np.array([np.random.randint(low=-input_shape[i], high=input_shape[i]) for i in range(rank)]).astype(np.int32)
        end_val = np.array([np.random.randint(low=-input_shape[i], high=input_shape[i]) for i in range(rank)]).astype(np.int32)
        stride_val = np.array([np.random.randint(low=-input_shape[i], high=input_shape[i]) for i in range(rank)]).astype(np.int32)
        if not masking:
        ...
        for i in range(rank):
            ...
            while True:
                ...
                normalized_end = input_shape[i] + end if end < 0 else end
                normalized_begin = input_shape[i] + begin_val[i] if begin_val[i] < 0 else begin_val[i]
        ...
        @make_tf_graph([input_shape, list(begin_val.shape)+[tf.int32], list(end_val.shape)+[tf.int32]])
        def build_model(x, begin, end):
            return tf.strided_slice(x, begin, end, stride_val,
        ...
        input_values = [np.array(list(range(np.prod(input_shape)))).reshape(input_shape).astype(np.float32),
                        begin_val, end_val]
        input_dict = dict(zip(inputs, input_values))
        run_compare_tf(model, input_dict, outputs,
               use_cpu_only=use_cpu_only,
    ...
    @pytest.mark.parametrize('use_cpu_only, backend, testcase',
                             itertools.product(
                                 [True, False],
                                 backends,
                                 # Change to slice representation for allowing iteration with a non-constant input
                                 [(slice(1,2), slice(1,2), slice(1,2)), # equivalent to [1:2, 1:2, 1:2]
                                    ...
                                    (slice(None), slice(1,None), slice(None,4,2)), # equivalent to [:,1:,:4:2]
...
class TestNonMaximumSuppression:
    ...
    def test_non_max_suppression(
            self, use_cpu_only, backend, num_boxes,
...
class TestPad:
    @pytest.mark.parametrize("use_cpu_only, backend, rank, mode, trial",
                             itertools.product(
                                 [True, False],
                                 backends,
                                 [2, 3, 4],
    ...
    def test(self, use_cpu_only, backend, rank, mode, trial):
        ...
        run_compare_tf(model, input_dict, outputs,
                       ...
                       frontend_only=False, backend=backend)
class TestPadV2:
    @pytest.mark.parametrize("use_cpu_only, backend, rank, constant_values",
                             itertools.product(
                                 [True, False],
                                 backends,
                                 list(range(1, 6)),
...
class TestConcat:
    ...
    def test_concat(self, use_cpu_only, backend, op_version):
        ...
        input_values = [np.random.rand(*input_shape1).astype(np.float32),
...
class TestPack:
    ...
    def test_pack(self, use_cpu_only, backend, rank, num_inputs):
        ...
        model, inputs, outputs = build_model
        input_values = [random_gen(shape, rand_min=-1, rand_max=1) for shape in input_shapes]
        input_dict = dict(zip(inputs, input_values))
        run_compare_tf(model, input_dict, outputs,
                       use_cpu_only=use_cpu_only,
                       frontend_only=False, backend=backend)
...
class TestReshape:
    ...
    @pytest.mark.parametrize("use_cpu_only, backend, input_shape",
                             itertools.product(
                                 [False],
                                 backends,
                                 [([10, 10], [5, 20]),
    ...
    @pytest.mark.parametrize("use_cpu_only, backend, shape",
                             itertools.product(
                                 [False],
                                 backends,
                                 [[1], [1,1], [1,1,-1], []],
...
class TestSpaceToBatchND:
    ...
    @pytest.mark.parametrize("use_cpu_only, backend, input_shape, block_shape, paddings",
                             itertools.product(
                                 [True, False],
                                 backends,
...
class TestBatchToSpaceND:
    # No direct mil smoke test since it's a TF op which is a composite of several ops.
    @pytest.mark.parametrize("use_cpu_only, backend, input_shape, block_size, crops",
                             itertools.product(
                                 [True, False],
                                 backends,
...
class TestLSTMBlockCell:
    @pytest.mark.parametrize("use_cpu_only, backend, batch, return_hc_only, has_peephole, has_clip",
                             itertools.product(
                                 [True, False],
                                 backends,
                                 [1, 2],
    ...
    def test_tf_no_variable(self, use_cpu_only, batch, backend, return_hc_only, has_peephole, has_clip):
        """
        ...
        with tf.Graph().as_default() as graph:
            x = tf.placeholder(tf.float32, shape=x_shape)
            res = _lstm_block_cell(x,
                    tf.constant(init_c),
                    tf.constant(init_h),
                    w=tf.constant(np.random.rand(input_dim+hidden_dim,
    ...
    def test_tf_lstm_block_cell(self, use_cpu_only, backend, batch):
        ...
        with tf.Graph().as_default() as graph:
            x = tf.placeholder(tf.float32, shape=x_shape)
            rnn_cell = tf.contrib.rnn.LSTMBlockCell(hidden_dim,
                    use_peephole=True,
                    forget_bias=np.random.rand())
