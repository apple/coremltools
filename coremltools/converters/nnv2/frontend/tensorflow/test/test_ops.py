...
class TestAddN:
    ...
    def test(self, use_cpu_only, backend, rank, num_inputs):
        if backend == 'nnv2_proto' and rank == 0:
            return
        input_shape = np.random.randint(low=1, high=6, size=rank)
...
class TestDepthwiseConv:
    @pytest.mark.parametrize(
        ...
        itertools.product(
            ...
            [(1, 1, 1, 1), (1, 2, 2, 1)],
            [(1, 1), (2, 2)],  # rdar://60668562 (NNv2: Conversion for TF op 'SpaceToBatchND' not implemented.)
            [True, False],
            [1, 3]
        ))
...
class TestElementWiseBinary:
    ...
    def test_binary(self, use_cpu_only, backend, rank, mode):
        # TODO: rdar://problem/63030405. Rank 0 tensor for NNV2
        if rank == 0 and backend == 'nnv2_proto':
            return
...
class TestReshape:
    ...
    def test_reshape(self, use_cpu_only, backend):
        def test_tf_static():
...
class TestSpaceToBatchND:
    # No direct nnv2 smoke test since it's a TF op which is a composite of several ops.
...
class TestLSTMBlockCell:
    ...
    def test_tf_no_variable(self, use_cpu_only, batch, backend, return_hc_only, has_peephole, has_clip):
        """
        If return_hc_only == True, the op can be mapped to cb.lstm.
        ...
        with tf.Graph().as_default() as graph:
            ...
            if return_hc_only:
                # All other outputs aren't supported by cb.lstm.
                res = res[1], res[6]
