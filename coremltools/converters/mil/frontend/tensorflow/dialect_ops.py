...
class tf_make_list(Operation):
    ...
    def type_inference(self):
        ...
        return types.list(elem_type, init_length=init_length,
                dynamic_length=self.dynamic_length.val)
...
class TfLSTMBase(Operation):
    ...
    def _check_peephole_weights(self):
        ...
        if self.use_peephole.val:
            if self.weight_peep_i is None or self.weight_peep_f is None or \
                    self.weight_peep_o is None:
                raise ValueError(
...
class tf_lstm_block_cell(TfLSTMBase):
    ...
    def type_inference(self):
        ...
        return (types.tensor(dtype, ret_shape), # i
               ...
               types.tensor(dtype, ret_shape),) # h
