...
class ReadMeSession():
	...
	def set_api_version(self, version):
		...
		if 'categories' not in self.get_version(): self.__refresh_categories()
	...
	def __api_call(self, api_func, endpoint, print_info, data=None):
		...
		response = api_func(_readme_api_url + endpoint,
	...
	def __api_GET(self, endpoint, print_info=None, paginated=False):
		if not print_info: print_info = "API::GET(" + endpoint + ")"
		if paginated:
			i = 1
			out = []
			while(True):
				response = self.__api_call(requests.get,
									 	   endpoint + "?page=" + str(i),
									 	   print_info + " (page " + str(i) + ")")
				if response is None: return None
				if len(response) is 0: return out
				out += response
				i += 1
		else:
	...
	def __api_POST(self, endpoint, data, print_info=None):
		if not print_info: print_info = "API::POST(" + endpoint + ")"

		# Convert data to str
		data_str = ""
		for x, y in data.items(): data_str += '"' + x + '":"' + y + '",'
	...
	def __refresh_versions(self):
		...
		if response:
			self.__versions = {}
			for version in response: self.get_versions()[version['version']] = version
	...
	def __refresh_categories(self):
		...
		response = self.__api_GET('categories',
								  paginated=True,
	...
	def __refresh_category_files(self, category):
		...
		category_files = self.__api_GET('categories/' + self.get_category(category)['slug'] + '/docs',
										print_info="Fetching docs in " + category)
		# Populate as a map: map<title, info object>>
		category = self.get_category(category)
		category['files'] = {}
		for file in category_files: category['files'][file['title']] = file
	...
	def create_version(self, version, from_version=None, is_stable=False, is_beta=False, is_hidden=True):
		if version in self.get_versions():
			raise ValueError("Version " + version + " already exists! Cannot create it.")
		...
		self.get_versions()[version] = self.__api_POST('version', data, "Creating version " + version)
	...
	def update_version(self, version, is_stable=None, is_beta=None, is_hidden=None):
		...
		data =  {
				   "version" 		  :  'v' + version,
				   "is_stable"        :  is_stable if is_stable is not None else self.get_versions()[version]['is_stable'],
				   "is_beta" 		  :  is_beta if is_beta is not None else self.get_versions()[version]['is_beta'],
				   "is_hidden" 		  :  is_hidden if is_hidden is not None else self.get_versions()[version]['is_hidden'],
				}
		version = self.__api_POST('version', data, "Creating version " + version)
		for k, v in version.items(): self.get_versions()[version][k] = v
	...
	def empty_category(self, category):
		...
		for title, data in self.get_category_files(category).items():
			self.__api_DELETE('docs/' + data['slug'],
	...
	def delete_file_with_title(self, title, category):
		...
		if title in files:
			self.__api_DELETE('docs/' + files[title]['slug'],
	...
	def upload(self, path, category, recursive=False):
		...
		if (os.path.isdir(path)):
			if recursive:
				...
				subdirpath = path
				onlydirs = [f for f in os.listdir(subdirpath) if os.path.isdir(os.path.join(subdirpath, f))]
				for dir in onlydirs: self.upload(os.path.join(path, dir),
										         category,
										         recursive)
		...
		if(currfilename.find('.html') != -1):
			...
			# Create the new page
			out = self.__api_POST('docs',
				data, "Uploading " + currfilename + " to category " + category)
