...
class TestExpandDims:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend):
        ...
        def build(x):
            return [mb.expand_dims(x=x, axes=[0]),
    ...
    def test_builder_to_backend_programmatic_one_axis(
        ...
        def build(x): return mb.expand_dims(x=x, axes=[axis])
    ...
    def test_builder_to_backend_programmatic_multiple_axes(
        ...
        def build(x): return mb.expand_dims(x=x, axes=axes)
...
class TestReshape:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend):
        ...
        input_values = {"x": t}
        def build(x):
            return [mb.reshape(x=x, shape=[3, 2]),
    ...
    def test_builder_to_backend_symbolic(self, use_cpu_only, backend):
        ...
        def build(x, shape, shape2):
            return [mb.reshape(x=x, shape=[2, -1]),
...
class TestTranspose:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend, is_symbolic):
        ...
        input_values = {"x": x}
        def build(x):
            return [mb.transpose(x=x, perm = (0,1)),
