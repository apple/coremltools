...
def backfill_make_list_elem_type(prog):
    """
    TF's TensorArrayV3 (represented as make_list in nnv2) doesn't necessarily
...
def backfill_make_list_elem_type_block(block):
    ...
    for op in block.operations[:]:
        ...
        if op.outputs[0].elem_type != builtins.unknown:
...
def infer_elem_type(list_var):
    """
    Returns builtins.tensor. None if failed to infer element type.
    Example:
