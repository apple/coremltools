...
#import "Caffe/CaffeConverter.hpp"
...
void convertCaffe(const std::string& srcPath,
    ...
    NSString *srcModelPath = [NSString stringWithUTF8String:srcPath.c_str()];
    if (![[NSFileManager defaultManager] fileExistsAtPath:srcModelPath]) {
        std::stringstream ss;
    ...
    if (classLabelPath != "") {
        NSString *classInputPath = [NSString stringWithUTF8String:classLabelPath.c_str()];
        if (![[NSFileManager defaultManager] fileExistsAtPath:classInputPath]) {
            std::stringstream ss;
