...
def AvgPool(context, node):
    ...
    if data_format == "NHWC":
        ...
        x = mb.avg_pool(x=x, kernel_sizes=kernel_sizes, strides=strides,
...
def Conv3D(context, node):
    ...
    x = mb.conv(x=x, weight=W_oidhw, pad_type=pad_type, strides=DHW_strides,
...
def FusedBatchNorm(context, node):
    ...
    if data_format == 'NHWC':
        ...
        x = _transpose_NHWC_to_NCHW(x)
        x = mb.batch_norm(x=x, mean=mean, variance=variance, gamma=scale,
...
def MatMul(context, node):
    ...
    x = mb.matmul(x=a, y=b, transpose_x=transpose_a,
...
def MaxPool(context, node):
    ...
    if data_format == "NHWC":
        x = _transpose_NHWC_to_NCHW(x)
        x = mb.max_pool(x=x, kernel_sizes=kernel_sizes, strides=strides,
                        pad_type=pad_type)
        x = _transpose_NCHW_to_NHWC(x, node.name)
    else:
        x = mb.max_pool(x=x, kernel_sizes=kernel_sizes, strides=strides,
...
def MirrorPad(context, node):
    ...
    pad = pad.reshape(-1)
    x = mb.pad(x=x, pad=pad, name=node.name + '_pad', constant_val=constant_val, mode=mode)
...
def ReverseSequence(context, node):
    ...
    x = mb.reverse_sequence(x=x, lengths=lengths, seq_axis=seq_axis,
...
def OneHot(context,node):
    ...
    x = mb.one_hot(indices=indices,one_hot_vector_size=depth, axis = axis,
...
def ResizeNearestNeighbor(context, node):
    ...
    x = mb.upsample_nearest_neighbor(x=x,
...
def While(context, node):
    ...
    def body(*loop_vars):
        ...
        context.unstack_func_inputs()
        return res
    x = mb.while_loop(_cond=cond, _body=body, loop_vars=loop_vars,
            name=node.name)
...
def iff(context, node):
    ...
    x = mb.cond(pred=pred, _true_fn=true_fn, _false_fn=false_fn,
                name=node.name)
    context.add(node.name, x)
...
def Unpack(context, node):
    ...
    for i in range(num_splits):
        output_vars.append(mb.squeeze(x=y[i], axes=[axis], name=node.name + ":{}".format(i)))
...
def TensorArrayV3(context, node):
    ...
    if elem_shape is not None:
        ls = mb.make_list(init_length=size, dtype=dtype_str,
                elem_shape=elem_shape, dynamic_length=dynamic_length,
                name=node.name)
    else:
        ls = mb.tf_make_list(init_length=size, dtype=dtype_str,
...
def TensorArrayWriteV3(context, node):
    ...
    new_list = mb.list_write(ls=ls, index=index, value=new_val,
            name=node.name)
...
def TensorArrayGatherV3(context, node):
    ...
    tensor = mb.list_gather(ls=ls, indices=indices,
            name=node.name)
...
def TensorArrayReadV3(context, node):
    ...
    ls = context[node.inputs[1]]
    ls = mb.list_read(ls=ls, index=idx,
            name=node.name)
    context.add(node.name, ls)
...
def TensorArrayScatterV3(context, node):
    ...
    value = context[node.inputs[1]]
    ls = context[node.inputs[2]]
    ls = mb.list_scatter(ls=ls, indices=indices,
...
def LSTMBlockCell(context, node):
    ...
    res = mb.tf_lstm_block_cell(x=x, c_prev=c_prev, h_prev=h_prev, weight=W,
            bias=bias,
...
def BlockLSTM(context, node):
    ...
    res = mb.tf_lstm_block(seq_len=seq_len, x=x, c_prev=init_c, h_prev=init_h,
