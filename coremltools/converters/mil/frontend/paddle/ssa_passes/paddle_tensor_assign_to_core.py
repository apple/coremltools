...
class paddle_tensor_assign_to_core(AbstractGraphPass):
    """
    Try to map Paddle dialect ops `paddle_tensor_assign` into core op set if compatible.
    ...
    Currently, we tranform the paddle_tensor_assign op into transposes + scatter + expand_dims

    Given:
        %output = paddle_tensor_assign(data=%data, updates=%updates)
    ...
    def apply(self, prog):
        for f in prog.functions.values():
            _paddle_tensor_assign_to_core_block(f)

@block_context_manager
def _paddle_tensor_assign_to_core_block(block):
    for op in block.operations[:]:
        for b in op.blocks:
            _paddle_tensor_assign_to_core_block(b)

        if op.op_type in ["paddle_tensor_assign"]:
