...
def try_replace_with_core_lstm(op):
    ...
    True if op can be represented by cb.lstm op in SSA. False otherwise
    """
    ...
    if op.use_peephole.val:
        cb_peep = np.stack([op.W_peep_i.val, op.W_peep_f.val,
            op.W_peep_o.val])

    # weights. TF1 W is icfo. Need to convert to ifco
    tf_w = op.W.val  # [input_dim+outut_dim, 4*hidden_dim] in icfo layout
    ...
    # for recurrence bias. nnv2 lstm expects bias shape [2, 4*hidden_dims]
    ...
    with block:
        ...
        if op.op_type == 'tf_lstm_block_cell':
            x = cb.expand_dims(x=op.x, axes=[0], before_op=op)
        else: # tf_lstm_block
            x = op.x
        new_h_all, new_h, new_cs = cb.lstm(x=x, initial_c=op.c_prev,
                initial_h=op.h_prev,
                weight=w,
                bias=bias,
                # activations[2] should be "sigmoid" (rdar://62272632)
                activations=("sigmoid", "tanh", "tanh"),
                peephole=cb_peep, clip=cell_clip,
