...
from .nnssa import NetworkEnsemble, ParsedNode, SSAFunction
...
class SSABuilder(object):
    ...
    def get_ssa(self):
        """
        Obtain the NitroSSA that can be used to do graph surgery/convert to backends.
        """
        return copy.deepcopy(self.ssa)

    def add_graph(self, graph=None, name="main"):
        """
        Add a function into NitroSSA by a constructed graph
        ...
        if name in self.ssa.functions:
            logging.warning("Failed adding graph! Name already exist in the NitroSSA network!")
        else:
            self.ssa.add_function(name, SSAFunction(graph))

    def add_function(self, function=None, name="main"):
        """
        Add a function into NitroSSA by SSAFunction
        ...
        if not isinstance(function, SSAFunction):
            logging.warning("Failed adding function! The input is not a SSAFunction.")
        elif name in self.ssa.functions:
            logging.warning("Failed adding function! Name already exist in the NitroSSA network!")
        else:
...
class GraphBuilder(object):
    ...
    def get_ssa(self, name='main'):
        """
        Obtain the NitroSSA that is derived from graph and can be used for
    ...
    def add_placeholder(self, init_from=None, datatype=None, name=None):
        ...
        init_from: numpy
            Can be used to derive the corresponding Nitro builtins type. Value
            will be ignored.
    ...
    def add_cast(self, input_name, datatype, name=None):
        """
        Add a 'cast' node to the SSAFunction.
        ...
        """
        return self._build_op('Cast', [input_name], name=name, attr={'DstT': datatype})
    ...
    def add_activation(self, op, input_name, name=None, attr=None):
        """
    ...
    def add_matmul(self, input_names, name=None, attr=None):
        """
    ...
    def add_const(self, value, name=None):
        ...
        SSANode form:
            op: 'Const'
            inputs: []
            value: Nitro.builtins
            datatype: Nitro.builtins type

        Parameters
        ----------
        value: Nitro.builtins types or numpy type.
    ...
    def add_split(self, split_dim, value, split_size="", num_split=0, name=None):
        ...
        Parameters
        ----------
        split_dim: str or int
            Input node which indicates the splitting axis (0-D tensor / int).
            This needs to be a constant node after all constant propagations.

        split_size: str or 1-D np.array
        ...
        """
        split_dim = self._maybe_add_const(split_dim, "split_dim")
        if num_split > 0:
            return self._build_op(
                'Split', [split_dim, value], name=name, attr={'num_split': num_split})
        else:
            split_size = self._maybe_add_const(split_size, "split_size")
            return self._build_op('SplitV', [value, split_size, split_dim], name=name)

    def add_slice(
            self, input_name, begin=None, end=None, size=None, strides=None, squeeze=None,
            name=None):
        """
        Add a slice node.
        ...
        begin: str or list of int
            ...
            Should have same length as the rank of input node.
        ...
        begin_mask: str or list of 0/1
            begin_mask[1] means to ignore begin[i]
            If str, the node must evaluate to constant and have the same
            length as the rank of input node.
    ...
    def add_reduction(self, op, input_name, axis, name=None, attr=None):
        """
        ...
        axis: list of int or node name (str)
            The axis that the operation would be performed. (Note that
            even though TF supports axis=None to reduce all axis, backend
            does not, so all axes need to be specified explicitly)
    ...
    def add_squeeze(self, input_name, squeeze_dims=None, name=None):
        """
    ...
    def add_expanddims(self, input_name, expand_dim, name=None):
        ...
        Parameters
        ----------
        input_name: str or int or np.array
            The name of the input_node
        ...
        name: str
            The name of this node
        """
        input_name = self._maybe_add_const(input_name, "expanddim_input")
    ...
    def add_transpose(self, input_name, axes, name=None):
        ...
        name: str
            The name of this node
        """
        axes = self._maybe_add_const(axes, "transpose_axes")
    ...
    def add_fill(self, shape, value, name=None):
        ...
        name: str
            The name of this node
        """
        shape = self._maybe_add_const(shape, "fill_shape")
        value = self._maybe_add_const(value, "fill_value")
    ...
    def add_while(self, input_name, body_function, cond_function, name=None):
        ...
        .. sourcecode:: python
            ...
            # This is the NitroSSA graph for a while loop.
            >>> ssa = sb.get_ssa()
    ...
    def add_range(self, start=None, stop=None, step=1, name=None):
        """
    ...
    def add_padding(self, input_name, paddings, constant_values=0.0, name=None):
        ...
        Parameters
        ----------
        input_name: str or list of np.array
            The name of the input_node
        ...
        name: str
            The name of this node
        """
        input_name = self._maybe_add_const(input_name, "padding_input")
        return self._build_op(
    ...
    def add_conv2D(
        ...
        padding: str
            Could be "SAME", "VALID", or "CUSTOM". If "CUSTOM", `pad` is required.

        pad: [t, b, l, r] (list of non-negative ints) for padding along H and
        W dimension.
    ...
    def add_tensorarray(self, array_size, datatype, shape, name=None):
        """
        Add a TensorArray (list of tensor) for SSAfunctions.
        The name of this TensorArray would also be the source name used by the
        read and writes.

        SSANode form:
            op: 'TensorArrayV3'
            inputs: [array_size]
            attr: 'dtype', 'element_shape'

        Examples
        --------
        .. sourcecode:: python

            # We first allocate a TensorArray of size 5.
            # This tensorarray would contain tensors of shape [2, 3]
            >>> TA = builder.add_tensorarray(5, builtins.float32, [2, 3])
            # Set a new input value
            >>> write_1 = builder.add_const(np.random.random([2, 3]))
            # op functions like TA[1] = write_1
            >>> write_op = builder.add_tensorarray_write(TA, 1, write_1)
            >>> write_op_1 = builder.add_tensorarray_write(TA, 0, np.random.random([2, 3]))
            # reading from TensorArray is also similar
            >>> read_1 = builder.add_tensorarray_read(TA, 1)
            # Or we can do multiple writes as scatter
            >>> scatter_1 = builder.add_const(np.random.random([2, 2, 3]))
            >>> scatter_op = builder.add_tensorarray_write(TA, [1, 0], scatter_1)

        Parameters
        ----------
        array_size: str
            The input_node, should be a int constant indicating the max size of
            the tensorarray. Set as 0 if the size is dynamic.

        datatype: builtins datatype
            The datatype that would be stored in the tensorarray.

        shape: list
            The shape of the tensor that would be pushed into the tensorarray.
        ...
        attr = {}
        attr['dtype'] = datatype
        attr['element_shape'] = shape
        attr['dynamic_size'] = False
        attr['infer_shape'] = True

        return self._build_op(
            'TensorArrayV3', [self._maybe_add_const(array_size)], attr=attr, name=name)

    def add_tensorarray_read(self, source, index, shape, name=None):
        """
        ...
        ----------
        source: str
            The tensorarray that is being read from.

        index: str or int
            Index to read from the tensorarray.
        ...
        return self._build_op(
            'TensorArrayReadV3', [self._maybe_add_const(index), source], name=name, attr=attr)

    def add_tensorarray_write(self, source, index, value, name=None):
        """
        Write to Tensorarray.

        SSANode form:
            op: 'TensorArrayWriteV3'
            inputs: [source, index, value]

        Parameters
        ----------
        source: str
            The tensorarray that will be updated.

        index: str or int
            Index to write to the tensorarray.

        value: str or ndarray
            Tensor to write to tensorarray.
        ...
        return self._build_op(
            'TensorArrayWriteV3',
            [self._maybe_add_const(index),
             self._maybe_add_const(value), source],
            name=name)

    def add_tensorarray_gather(self, source, indices, shape, name=None):
        """
        Gather from Tensorarray.
        ...
        indices: str or int
            Indices to read from the tensorarray.
        ...
        return self._build_op(
            'TensorArrayGatherV3', [self._maybe_add_const(indices), source], name=name, attr=attr)

    def add_tensorarray_scatter(self, source, indices, values, name=None):
        """
        Scatter to Tensorarray.

        SSANode form:
            op: 'TensorArrayScatterV3'
            inputs: [source, indices, values]
        ...
        indices: str or int
            Indices to write to the tensorarray.

        values: str or ndarray
            Tensors to write to tensorarray.
        ...
        return self._build_op(
            ...
            [self._maybe_add_const(indices),
             self._maybe_add_const(values), source],
            name=name)

    def add_tensorarray_size(self, source, name=None):
        """
        Return size of Tensorarray.
        ...
        ----------
        source: str
            The tensorarray's size to be returned.

        name: str
            The name of this node
        """

        return self._build_op('TensorArraySizeV3', [source])
    ...
    def add_LSTMBlock(
        ...
        .. sourcecode:: python
            ...
            # Similary, we can just use cell for each timestep if we want.
        ...
        prev_cs (optional): str [batch_size, {1,2}*hidden_size]
    ...
    def add_one_hot(self, val, idx, dim, background_val=0, name=None):
        """
        val (str or [int]): the non-zero value
        idx (str or [int]): idx-th entry will have val.
        dim (str or [int]): vector dim
        background_val (float): the value for all positions besides idx
        Note: `val`, `idx`, `dim` have to resolve to an [int] (1-D tensor) after
        type inference.

        Return ssa node representing one-hot 1D tensor [0, 0, ..., val, ...0]
        (0 = background_val)
        """
        dim_minus_1 = self.add_elementwise("Sub", [dim, 1])  # r-1 (rank-0)
        num_right = self.add_elementwise("Sub", [dim_minus_1, idx])  # shape: [1,]
        # zeros_left.shape: [num_left,]
        zeros_left = self.add_fill(idx, background_val)
        zeros_right = self.add_fill(num_right, background_val)
        return self.add_concat([zeros_left, val, zeros_right], axis=0, name=name)

    def _maybe_add_const(self, var, name=None):
        """
        ...
        if not hasattr(self, name):
            setattr(self, name, 0)
        node_name = self.prefix + name + str(getattr(self, name))
        ...
        if isinstance(var, list):
            dtype = None
            if len(var) > 0:
                if isinstance(var[0], np.generic):
                    dtype = var[0].dtype
                elif isinstance(var[0], six.integer_types):
                    dtype = np.int32
                elif isinstance(var[0], float):
                    dtype = np.float32
            return self.add_const(np.asarray(var, dtype=dtype), name=node_name)
