...
class TestActivation:
    ...
    def test_activation(self, use_cpu_only, backend, rank, op):
        kwargs = {"atol": 1e-3, "rtol": 1e-4} if \
            op == tf.keras.activations.exponential and \
            use_cpu_only is False else {}
        ...
        run_compare_tf_keras(
            model, [random_gen(shape, -10, 10)],
            use_cpu_only=use_cpu_only, backend=backend, **kwargs)
...
class TestBinary:
    ...
    def test_dot(self, use_cpu_only, rank, backend, axes, normalize):
        ...
        input_y = tf.keras.layers.Input(
            batch_input_shape=tuple(shape))
        out = tf.keras.layers.Dot(axes=axes, normalize=normalize)(
            [input_x, input_y])
        model = tf.keras.Model(inputs=[input_x, input_y], outputs=out)
        run_compare_tf_keras(
            model, [random_gen(shape, -10, 10),
...
class TestConcatenate:
    ...
    def test(self, use_cpu_only, backend, rank, axis):
        ...
        run_compare_tf_keras(
            model, [random_gen(shape), random_gen(shape)],
            use_cpu_only=use_cpu_only, backend=backend)
...
class TestConvolution:
    @pytest.mark.parametrize(
        ...
        itertools.product(
            ...
            [(1, 1, 1), (1, 2, 3), (1, 3, 2)],
            [(1, 1, 1)],  # rdar://62951360 (Enhance SpaceToBatchND op to support more dialation rate of Conv)
            [1, 3]
        ))
    def test_conv(
            self, use_cpu_only, backend, op, padding, data_format,
            spatial_dim_and_ks, strides, dilations, batch_size):
        s1, s2, s3, k1, k2, k3 = spatial_dim_and_ks
        c_in, c_out = 2, 3
        input_shape = None
        kernel_size = None
        if op in {tf.keras.layers.Conv1D,
                  tf.keras.layers.LocallyConnected1D}:
            input_shape = (batch_size, s3, c_in)
            kernel_size = k3
            strides = strides[2]
            dilations = dilations[2]
        elif op in {tf.keras.layers.Conv2D,
                    tf.keras.layers.LocallyConnected2D}:
        ...
        if op in {tf.keras.layers.LocallyConnected1D,
                  tf.keras.layers.LocallyConnected2D}:
            if padding != "valid":
        ...
        run_compare_tf_keras(
            model, [random_gen(input_shape, rand_min=-10, rand_max=10)],
            use_cpu_only=use_cpu_only, backend=backend)
    ...
    def test_conv_transpose(
            self, use_cpu_only, backend, op, padding, data_format,
            spatial_dim_and_ks, output_padding, strides, dilations,
            batch_size):
        s1, s2, s3, k1, k2, k3 = spatial_dim_and_ks
        c_in, c_out = 2, 3
        input_shape = None
...
class TestEmbedding:
    ...
    def test(self, use_cpu_only, backend, dims, batch_size, input_length):
        ...
        run_compare_tf_keras(
            model, [random_gen(shape, rand_min=0, rand_max=dims[0])],
            use_cpu_only=use_cpu_only, backend=backend,
            atol=1e-3, rtol=1e-4)
...
class TestLambda:
    ...
    def test_unary(self, use_cpu_only, backend, rank, function):
        ...
        run_compare_tf_keras(
            model, [random_gen(shape, rand_min=-5, rand_max=5)],
            use_cpu_only=use_cpu_only, backend=backend)
...
class TestPermute:
    ...
    @pytest.mark.parametrize(
        ...
        itertools.product(
            [True, False],
            backends,
            [(rank, perm) for rank in range(3, 6)
    ...
    def test(self, use_cpu_only, backend, rank_and_perm):
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.Permute(
                batch_input_shape=shape, dims=perm
            )])
        run_compare_tf_keras(
            model, [random_gen(shape, rand_min=-10, rand_max=10)],
            use_cpu_only=use_cpu_only, backend=backend)
...
class TestPooling:
    ...
    def test_global_pooling(
            self, use_cpu_only, backend, op, data_format):
        shape = None
        if op in {
            tf.keras.layers.GlobalAveragePooling1D,
            tf.keras.layers.GlobalMaxPool1D}:
    ...
    def test_pooling(
            self, use_cpu_only, backend, op, data_format, pool_size):
        shape = None
        if op in {
            tf.keras.layers.AveragePooling1D,
            tf.keras.layers.MaxPool1D}:
...
class TestRecurrent:
    ...
    @pytest.mark.parametrize(
        ...
        itertools.product(
            ...
            [True, False]))
    def test_lstm(
            self, use_cpu_only, backend, rank, units, activation,
            recurrent_activation, use_bias, return_sequences):
    ...
    def test_lstmcell(self, use_cpu_only, backend):
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.RNN(
                batch_input_shape=shape,
                cell=tf.keras.layers.LSTMCell(units=3))
        ])
        run_compare_tf_keras(
            model, [random_gen(shape, rand_min=-1, rand_max=1)],
            use_cpu_only=use_cpu_only, backend=backend)
...
class TestUpSampling:
    ...
    def test(
            self, use_cpu_only, backend, op,
            upsample_factor, data_format, interpolation):
        kwargs = {}
        shape = None
