...
def _generate_data(input_shape, mode="random", scale=2.0 / 255, bias=-1, img_size=256):
    """
...
class CorrectnessTest(unittest.TestCase):
    ...
    """
        ...
        with tf.Session(graph=g) as sess:
            ...
            for idx, in_tensor in enumerate(input_tensors):
                ...
                feed_dict[ts_name] = _generate_data(
                    ts_shape,
                    mode=data_modes[idx],
                    scale=scale,
                    bias=bias,
        ...
        for idx, in_tensor in enumerate(input_tensors):
            ...
            if in_tensor_name in sequence_inputs:
                coreml_inputs[coreml_in_name] = _tf_transpose(
                    feed_dict["import/" + in_tensor_name], is_sequence=True
                ).copy()
            else:
                coreml_inputs[coreml_in_name] = _tf_transpose(
                    feed_dict["import/" + in_tensor_name]
                ).copy()
    ...
    def _test_coreml_model_image_input(
        self,
        tf_model_path,
        coreml_model,
        input_tensor_name,
    ...
    """
        ...
        if SupportedVersion.is_nd_array_supported(minimum_ios_deployment_target):
            ...
            coreml_output_name = output_tensor_name.replace(":", "__").replace(
                "/", "__"
            )
...
class TestModels(CorrectnessTest):
    ...
    def test_googlenet_resnet_v2(self):
        ...
        tf_model_path = os.path.join(
            TMP_MODEL_DIR, "inception_resnet_v2_2016_08_30_frozen.pb"
        )

        mlmodel_path = os.path.join(
            TMP_MODEL_DIR, "inception_resnet_v2_2016_08_30_frozen.mlmodel"
        )
        mlmodel = tf_converter.convert(
            filename=tf_model_path,
            mlmodel_path=mlmodel_path,
    ...
    def test_googlenet_v1_slim(self):
        ...
        mlmodel_path = os.path.join(
            TMP_MODEL_DIR, "inception_v1_2016_08_28_frozen.mlmodel"
        )
        mlmodel = tf_converter.convert(
            filename=tf_model_path,
            mlmodel_path=mlmodel_path,
    ...
    def test_googlenet_v1_slim_with_img_format(self):
        ...
        with self.assertWarns(Warning):
            ...
            )  # Should not be used by legacy converter, expect warning.
    ...
    def test_googlenet_v1_slim_coreml_3(self):
        ...
        mlmodel = tf_converter.convert(
            filename=tf_model_path,
            mlmodel_path=mlmodel_path,
            outputs=["InceptionV1/Logits/Predictions/Softmax"],
    ...
    def test_googlenet_v2_slim(self):
        ...
        mlmodel_path = os.path.join(
            TMP_MODEL_DIR, "inception_v2_2016_08_28_frozen.mlmodel"
        )
        mlmodel = tf_converter.convert(
            filename=tf_model_path,
            mlmodel_path=mlmodel_path,
    ...
    def test_googlenet_v4_slim(self):
        ...
        mlmodel_path = os.path.join(
            TMP_MODEL_DIR, "inception_v4_2016_09_09_frozen.mlmodel"
        )
        mlmodel = tf_converter.convert(
            filename=tf_model_path,
            mlmodel_path=mlmodel_path,
    ...
    def test_mobilenet_v1_100_224(self):
        ...
        tf_model_dir = _download_file(url=url)
        tf_model_path = os.path.join(
            TMP_MODEL_DIR, "mobilenet_v1_1.0_224/frozen_graph.pb"
        )
    ...
    def test_mobilenet_v1_75_192(self):
        ...
        tf_model_dir = _download_file(url=url)
        tf_model_path = os.path.join(
            TMP_MODEL_DIR, "mobilenet_v1_0.75_192/frozen_graph.pb"
        )
    ...
    def test_mobilenet_v1_50_160(self):
        ...
        tf_model_dir = _download_file(url=url)
        tf_model_path = os.path.join(
            TMP_MODEL_DIR, "mobilenet_v1_0.50_160/frozen_graph.pb"
        )
    ...
    def test_style_transfer(self):
        ...
        mlmodel = tf_converter.convert(
            ...
            mlmodel_path=mlmodel_path,
            outputs=["Squeeze:0"],
            inputs={"input:0": [1, 256, 256, 3], "style_num:0": [26]},
        )

        # Test predictions on an image
        input_tensors = [("input:0", [1, 256, 256, 3]), ("style_num:0", [26])]
