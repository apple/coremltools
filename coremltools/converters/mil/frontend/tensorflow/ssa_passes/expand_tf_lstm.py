...
def expand_tf_lstm_block_cell(op):
    ...
    with op.enclosing_block as block:
        ...
        xh = mb.concat(values=[x, h_prev], axis=1, before_op=op)

        # w: [4*hidden_dim, input_dim + hidden_dim] (icfo layout)
        w = np.transpose(op.weight.val)
        # [i, ci, f, o] = xh * w + b. Shape is [b, 4*hidden_dim]
        icfo = mb.linear(x=xh, weight=w, bias=b, before_op=op)

        # i, ci, f, o shape: [b, hidden_dim]
        i, ci, f, o = mb.split(x=icfo, num_splits=4, axis=1, before_op=op)
        #i, f, ci, o = mb.split(x=icfo, num_splits=4, axis=1, before_op=op)
        if op.forget_bias.val != 0:
            f = mb.add(x=f, y=forget_bias, before_op=op)
        ...
        if op.use_peephole.val:
            ...
            x = mb.mul(x=cs_prev, y=wci, before_op=op)
            pre_i = mb.add(x=x, y=i, before_op=op)

            x = mb.mul(x=cs_prev, y=wcf, before_op=op)
            pre_f = mb.add(x=x, y=f, before_op=op)
        else:
            pre_i = i
            pre_f = f

        i = mb.sigmoid(x=pre_i, before_op=op)
        f = mb.sigmoid(x=pre_f, before_op=op)

        # ci = tanh(ci)
        ci = mb.tanh(x=ci, before_op=op)

        # cs = ci .* i + cs_prev .* f
        x = mb.mul(x=ci, y=i, before_op=op)
        y = mb.mul(x=cs_prev, y=f, before_op=op)
        cs = mb.add(x=x, y=y, before_op=op)

        # cs = clip(cs, cell_clip)
        if op.cell_clip is not None:
            clip_val = op.cell_clip.val
            cs = mb.clip(x=cs, alpha=-clip_val, beta=clip_val, before_op=op)

        # o = sigmoid(cs * wco + o)
        if op.use_peephole.val:
            wco = op.weight_peep_o.val
            x = mb.mul(x=cs, y=wco, before_op=op)
            pre_o = mb.add(x=x, y=o, before_op=op)
        else:
            pre_o = o
        o = mb.sigmoid(x=pre_o, before_op=op)

        # co = tanh(cs)
        co = mb.tanh(x=cs, before_op=op)

        # h = co .* o
        h = mb.mul(x=co, y=o, before_op=op)
