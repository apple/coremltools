...
class TestList:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend):
        ...
        def build(a, b):
            ...
            ls = mb.list_write(ls=ls, index=0,
                    value=a)
            # this write is out of bound
            ls = mb.list_write(ls=ls, index=4,
                    value=b)
            ls = mb.list_scatter(ls=ls,
                    indices=[2, 1],
                    value=np.array([[-1, -2],
                                    [-4, -5]], dtype=np.float32))
            return init_t, mb.list_read(ls=ls, index=0), \
    ...
    def test_builder_to_backend_while(self, use_cpu_only, backend):
        ...
        def body(i, num_iters, ls, update):
            ...
            return mb.add(x=i, y=1), num_iters, \
                    mb.list_write(ls=ls, index=i,
        ...
        def build(num_iters, update):
            ...
            _, _, final_tensor_list, _ = mb.while_loop(_cond=cond, _body=body,
            ...
            return mb.list_gather(ls=final_tensor_list,
                    indices=indices)
