...
from torch.ao.quantization.pt2e.utils import get_aten_graph_module as _get_aten_graph_module
...
# All activations for which fusion is supported
...
def _adjust_activation_qspec(
    ...
    is_always_affine_op = node.target in always_affine_ops or (
        node.target in [_torch.ops.aten.hardtanh.default, _torch.ops.aten.hardtanh_.default]
...
def _get_weighted_mod_bn_pattern(
    ...
    def pattern(input, weight, bias, bn_weight, bn_bias, bn_run_mean, bn_run_var):
        mod_out = mod_fn(input, weight, bias)
        output = _F.batch_norm(mod_out, bn_run_mean, bn_run_var, bn_weight, bn_bias, training=True)
        if act_fn is not None:
            output = act_fn(output, inplace=True) if act_in_place else act_fn(output)
        return output, {
            "input": input,
...
def get_conv_pattern(
    ...
    return _get_weighted_mod_pattern(_conv_fn_map[conv_dim], example_inputs, act_fn, act_in_place)
...
def annotate_weighted_mod_pattern(
    ...
    for match in matches:
        ...
        if mod_node.args[0] is not input_node:
            raise ValueError(f"Weighted module arg did not contain input node {input_node}")
        if mod_node.args[1] is not weight_node:
            raise ValueError(f"Weighted module arg did not contain weight node {weight_node}")
        if len(mod_node.args) > 2 and mod_node.args[2] is not bias_node:
            raise ValueError(f"Weighted module arg did not contain bias node {bias_node}")
        ...
        if not _is_annotated([input_node]):
            ...
            input_qspec_map[input_node] = input_node.meta["quantization_annotation"].output_qspec

        input_qspec_map[weight_node] = quantization_config.weight if quantization_config else None
        output_qspec = quantization_config.output_activation if quantization_config else None
        if bias_node is not None:
        ...
        if output_node is None:
            ...
            if not _is_annotated([output_node]):
                output_qspec = _adjust_activation_qspec(node=output_node, qspec=output_qspec)
...
def annotate_binary_op_act_pattern(
    ...
    for match in matches:
        ...
        input_act_qspec = quantization_config.input_activation if quantization_config else None
        output_act_qspec = quantization_config.output_activation if quantization_config else None
        ...
        if output_node is None:
            ...
            output_act_qspec = _adjust_activation_qspec(node=output_node, qspec=output_act_qspec)
            output_node.meta["quantization_annotation"] = _QuantizationAnnotation(
                output_qspec=output_act_qspec,
                _annotated=True,
            )
...
def annotate_conv_bn_act_helper(
    ...
    for conv_dim, act_fn, act_in_place in combinations:
        ...
        annotated_partitions.extend(
            annotate_weighted_mod_pattern(model, pattern_gm, quantization_config, filter_fn)
        )
...
def annotate_binary_op_helper(
    ...
    for binary_op, act_fn, act_in_place in combinations:
        ...
        annotated_partitions.extend(
            annotate_binary_op_act_pattern(model, pattern_gm, quantization_config, filter_fn)
        )
