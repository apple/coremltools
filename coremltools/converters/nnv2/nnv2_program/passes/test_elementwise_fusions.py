...
class TestElementwiseOptimizationPasses:
    ...
    @pytest.mark.parametrize("conv_dim, \
    ...
    def test_fuse_bias_conv(self,
        ...
        @cb.program(input_specs=[cb.TensorSpec(shape=input_shape)])
        def prog(x):
            kwargs = {'x':x, 'pad_type':"valid", 'dilations':[1]*conv_dim, 'strides':[1]*conv_dim}
            if prebuilt_bias:
                if use_conv_transpose:
                    kwargs['bias'] = np.random.rand(Cout)
                else:
                    kwargs['B'] = np.random.rand(Cout)

            if use_conv_transpose:
                x = cb.conv_transpose(weight=W, **kwargs)
            else:
                x = cb.conv(W=W, **kwargs)

            if use_sub_instead:
                x = cb.sub(x=x, y=const)
            else:
                x = cb.add(x=const if flip_add_input_order else x, y= x if flip_add_input_order else const)
            x = cb.relu(x=x)
            return x
        ...
        bias_identifier = 'B' if not use_conv_transpose else 'bias'
        ...
        old_bias = prev_block.find_ops(op_type=conv_op)[0].inputs.get(bias_identifier, None)
        old_bias_val = 0 if old_bias is None else old_bias.val
        assert old_bias_val is not None
        assert block.find_ops(op_type=conv_op)[0].inputs[bias_identifier] is not None
        new_bias_val = block.find_ops(op_type=conv_op)[0].inputs[bias_identifier].val
    ...
    @pytest.mark.parametrize("flip_mul_input_order, flip_add_input_order, rank_3_const_input",
                             itertools.product(
                                 [False, True],
                                 [False, True],
                                 [False, True]
    ...
    def test_mul_add_fusion_to_batchnorm(self, flip_mul_input_order,
        ...
        @cb.program(input_specs=[cb.TensorSpec(shape=(1,10,10,C))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0,3,1,2])
            if flip_mul_input_order:
                x = cb.mul(x=gamma, y=x)
            else:
                x = cb.mul(x=x, y=gamma)
        ...
        assert_model_is_valid(prog, {'x': (1, 10, 10, C)},
