...
class DotVisitor(object):
    ...
    def visit(self, block, op, nodename_prefix=""):
        ...
        if op.name in self.highlights and op.name not in [o.name for o in block.outputs]:
            self.result.append(
                '"' + nodename_prefix + "op: " + op.name + '"' + '[label="' + label +
                '",fillcolor=%s,style=filled,fontcolor=%s]' % (self.highlights[op.name],'violetred'))
        else:
            self.result.append(
                '"' + nodename_prefix + "op: " + op.name + '"' + '[label="' + label + '",fontcolor=%s]' % ('violetred'))
        ...
        for input_var in _get_input_vars(op, only_nonconst_vars=True):
            ...
            edge = '"' + nodename_prefix + input_name + '"' + " -> " + '"' + nodename_prefix + "op: " + op.name + '"'
    ...
    def visit_input_var(self, var, nodename_prefix=''):
        ...
        if var.name in self.highlights:
            self.result.append(
                '"' + nodename_prefix + var.name + '"' + '[label="' + label +
                '",fillcolor=%s,style=filled,fontcolor=%s]' % (self.highlights[var.name],'violetred'))
        else:
            self.result.append(
                '"' + nodename_prefix + var.name + '"' + '[label="' + label + '",fontcolor=%s]' % ('violetred'))
    ...
    def visit_output_vars(self, block, var, nodename_prefix=''):
        ...
        parent_op = var.op
        edge = '"' + nodename_prefix + "op: " + parent_op.name + '"' + " -> " + '"' + nodename_prefix + var.name + '"'
        self.result.append(edge)
