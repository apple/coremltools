...
The example shows how you can use the MLModel object returned by the conversion to update the metadata and input/output descriptions that are displayed in the Xcode preview. (For details about the Xcode preview, see [Xcode Model Preview Types](doc:xcode-model-preview-types).
