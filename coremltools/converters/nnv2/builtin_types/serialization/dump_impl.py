...
from .typedefs import dump_np_types, py_types


def _dump_impl(obj, writer, expected_type=None):
    if isinstance(obj,
                  (np.bool_, np.integer, six.integer_types)) or issubclass(
                      type(obj), int):
        assert (expected_type is None
                or builtins.get_type_info(expected_type).name == 'int')
        writer.write_byte(py_types.int.value)
        writer.write_int(obj)
    elif isinstance(obj, bytes):
        # str == bytes in python 2
        assert (expected_type is None
                or builtins.get_type_info(expected_type).name == 'str')
        ...
        assert (expected_type is None
                or builtins.get_type_info(expected_type).name == 'double')
        writer.write_byte(py_types.double.value)
        writer.write_double(obj)
    elif isinstance(obj, list):
        assert (expected_type is None
                or builtins.get_type_info(expected_type).name == 'list')
        ...
        assert (expected_type is None
                or builtins.get_type_info(expected_type).name == 'dict')
        ...
        slot_and_types = sorted(zip(obj.__slots__, obj.__slot_types__),
                                key=lambda x: x[0])
