...
def try_to_transform(conv_op, add_op, block):
    ...
    weight_identifier = 'W' if conv_op.op_type == 'conv' else 'weight'
    bias_identifier = 'B' if conv_op.op_type == 'conv' else 'bias'
    ...
    old_bias = conv_op.inputs.get(bias_identifier, None)
    old_bias_value = None
    ...
    if old_bias is None:
        ...
        if np.prod(bias_value.shape) == 1:
            ...
            if conv_op.inputs[weight_identifier].val is None:
                return False
            if conv_op.op_type == 'conv':
                Cout = conv_op.inputs[weight_identifier].val.shape[0]
            else:
                Cout = conv_op.inputs[weight_identifier].val.shape[-2]
    ...
    new_bias_var = cb.const(val=new_bias_value, mode='file_value', before_op=conv_op)

    conv_kargs = {bias_identifier:new_bias_var, 'name':out_name, 'before_op':conv_op}

    for k,v in conv_op.inputs.items():
        if k == bias_identifier:
            continue
        conv_kargs[k] = v

    if conv_op.op_type == 'conv':
        x = cb.conv(**conv_kargs)
    else:
        x = cb.conv_transpose(**conv_kargs)
