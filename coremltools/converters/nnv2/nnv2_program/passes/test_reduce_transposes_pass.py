...
class TransposeOptimizationPass(unittest.TestCase):
    ...
    def test_simple_consecutive_ops_fusion(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(10, 20))])
        def prog(x):
            x = cb.transpose(x=x, perm=[1, 0])
            x = cb.transpose(x=x, perm=[1, 0])
            x = cb.relu(x=x)
            return x
    ...
    def test_linear_graph_two_op_fusion(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(1, 2, 3, 4))])
        def prog(x):
    ...
    def test_fusion_with_output_edge_inbetween(self):
        ...
        def prog(x):
            ...
            x1 = cb.relu(x=x)
            x2 = cb.log(x=x1)
            x3 = cb.transpose(x=x2, perm=[0, 2, 3, 1])
            x4 = cb.relu(x=x3)
            return x4, x1
    ...
    def test_multiple_fusions(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(10, 2, 3))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 2, 1])
            x1 = cb.relu(x=x)
            x2 = cb.relu(x=x)
            y1 = cb.transpose(x=x1, perm=[0, 2, 1])
            x3 = cb.log(x=x2)
        ...
        assert_model_is_valid(prog, {'x': (10, 2, 3)},
                              expected_output_shapes={block.outputs[0].name: (10, 2, 3),
    ...
    def test_partial_fusion_0(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(10, 2, 3, 5))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 2, 3, 1])
            x1 = cb.relu(x=x)
            x2 = cb.relu(x=x)
            y1 = cb.avg_pool(x=x1, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
            x3 = cb.log(x=x2)
        ...
        assert_model_is_valid(prog, {'x': (10, 2, 3, 5)},
                              expected_output_shapes={block.outputs[0].name: (10, 3, 5, 2),
    ...
    def test_partial_fusion_1(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(10, 2, 3, 5))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 2, 1, 3])
            x1 = cb.relu(x=x)
            x2 = cb.avg_pool(x=x, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
            y1 = cb.transpose(x=x1, perm=[0, 2, 1, 3])
            x3 = cb.log(x=x2)
        ...
        assert_model_is_valid(prog, {'x': (10, 2, 3, 5)},
                              expected_output_shapes={block.outputs[0].name: (10, 2, 3, 5),
                                                      block.outputs[1].name: (10, 2, 3, 5)})
    ...
    def test_partial_fusion_2(self):
        ...
        def prog(x):
            x = cb.relu(x=x)
            x = cb.transpose(x=x, perm=[0, 2, 1, 3])
            y1 = cb.transpose(x=x, perm=[0, 2, 1, 3])
            x1 = cb.relu(x=x)
            y2 = cb.transpose(x=x1, perm=[0, 2, 1, 3])
            y3 = cb.avg_pool(x=x1, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
    ...
    def test_partial_fusion_3(self):
        ...
        def prog(x):
            x = cb.relu(x=x)
            x = cb.transpose(x=x, perm=[0, 2, 1, 3])
            x1 = cb.transpose(x=x, perm=[0, 2, 1, 3])
            x2 = cb.relu(x=x)
            return x1, x2
    ...
    def test_partial_fusion_4(self):
        ...
        def prog(x):
            x = cb.relu(x=x)
            out2 = cb.transpose(x=x, perm=[0, 2, 1, 3])
    ...
    def test_no_fusion_more_materialization_ops(self):
        ...
        def prog(x):
            ...
            y2 = cb.transpose(x=x1, perm=[0, 2, 1, 3])
            x2 = cb.relu(x=x1)
            y3 = cb.avg_pool(x=x2, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
            y4 = cb.avg_pool(x=x1, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
            return y2, y3, y4
    ...
    def test_fusion_with_axis_op(self):
        ...
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 2, 1])
            x1 = cb.relu(x=x)
            x2 = cb.reduce_mean(x=x, axes=[2], keep_dims=True)
            y1 = cb.transpose(x=x1, perm=[0, 2, 1])
            x3 = cb.log(x=x2)
            y2 = cb.transpose(x=x3, perm=[0, 2, 1])
            return y1, y2
    ...
    def test_fusion_with_pad_reflective_op_0(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(11, 2, 3, 6))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 3, 1, 2])
            x2 = cb.pad(x=x, pad=[0,0,0,0,1,2,3,4], mode='reflect')
            x3 = cb.log(x=x2)
        ...
        assert_model_is_valid(prog, {'x': (11, 2, 3, 6)},
    ...
    def test_fusion_with_pad_reflective_op_1(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(11, 2, 3, 6))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 1, 3, 2])
            x2 = cb.pad(x=x, pad=[0,0,0,0,1,2,3,4], mode='reflect')
            x3 = cb.log(x=x2)
    ...
    def test_fusion_with_pad_constant_op(self):
        ...
        def prog(x):
            x = cb.transpose(x=x, perm=[0, 3, 1, 2])
            x2 = cb.pad(x=x, pad=[0,0,0,0,1,2,3,4], mode='constant', constant_val=3.0)
            x3 = cb.log(x=x2)
            y2 = cb.transpose(x=x3, perm=[0, 2, 3, 1])
            return y2
    ...
    def test_fusion_with_add_constant_op(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(1,2,5,5))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0,2,3,1])
            x = cb.add(x=x, y=np.array([10,100]))
            x = cb.transpose(x=x, perm=[0,3,1,2])
            return x
        ...
        assert_model_is_valid(prog, {'x': (1, 2, 5, 5)},
    ...
    def test_fusion_with_add_broadcastable_0(self):
        ...
        def prog(x):
            ...
            x2 = cb.relu(x=x)
            x2 = cb.transpose(x=x2, perm=[0, 2, 3, 1])
            x3 = cb.add(x=x1, y=x2)
    ...
    def test_fusion_with_add_broadcastable_1(self):
        ...
        def prog(x):
            x1 = cb.transpose(x=x, perm=[0, 2, 3, 1])
            x2 = cb.transpose(x=x, perm=[0, 2, 3, 1])
            x3 = cb.add(x=x1, y=x2)
            y = cb.transpose(x=x3, perm=[0, 3, 1, 2])
            return y
    ...
    def test_concat_pattern_0(self):
        ...
        def prog(x):
            ...
            x1 = cb.relu(x=x1)
            x2 = cb.relu(x=x2)
            x3 = cb.concat(values=[x1,x2],axis=3)
    ...
    def test_concat_pattern_5(self):
        ...
        @cb.program(input_specs=[cb.TensorSpec(shape=(10, 20, 30))])
        def prog(x):
            x1 = cb.transpose(x=x, perm=[2, 0, 1])
            c = cb.const(val=const)
            x2 = cb.concat(values=[x1,c],axis=2)
    ...
    def test_skip_connection_pattern_0(self):
        @cb.program(input_specs=[cb.TensorSpec(shape=(1, 5, 5, 3))])
        def prog(x):
            x = cb.transpose(x=x, perm=[0,3,1,2])
            x = cb.relu(x=x)
            x1 = cb.transpose(x=x, perm=[0,2,3,1])
        ...
        assert_model_is_valid(prog, {'x': (1, 5, 5, 3)},
    ...
    def test_skip_connection_pattern_1(self):
        ...
        def prog(x):
            ...
            x3 = cb.transpose(x=x2, perm=[0, 3, 1, 2])
            x4 = cb.add(x=x, y=x3)
            x4 = cb.transpose(x=x4, perm=[0, 2, 3, 1])
            x5 = cb.relu(x=x4)
            x6 = cb.avg_pool(x=x5, kernel_sizes=[1, 1], strides=[1, 1], pad_type='valid')
            return x6
