...
class TestRandomBernoulli:
    ...
    def test_builder_to_backend_stress(self, use_cpu_only, backend, rank, prob, dynamic):
        ...
        x_val = np.array([0.], dtype=np.float32)
        if dynamic:
            input_placeholders = {'x': mb.placeholder(shape=x_val.shape), 'dyn_shape': mb.placeholder(shape=shape.shape, dtype=types.int32)}
        ...
        def build(x):
            return [
                mb.add(x=x, y=x),
                mb.random_bernoulli(shape=shape, prob=prob)
            ]

        def build_dyn(x, dyn_shape):
            return [
                mb.add(x=x, y=x),
                mb.random_bernoulli(shape=dyn_shape, prob=prob)
            ]
        ...
        if dynamic:
            expected_output_types = [tuple([UNK_SYM for _ in o.shape]) + (types.fp32,) for o in expected_outputs]
        else:
            expected_output_types = [o.shape[:] + (types.fp32,) for o in expected_outputs]
...
class TestRandomNormal:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend):
        ...
        def build(x):
            return [
                mb.add(x=x, y=x),
                mb.random_normal(shape=np.array([2, 1, 3], np.int32), mean=1., stddev=0.),
                mb.random_normal(shape=np.array([3, 1, 2], np.int32), mean=0., stddev=0.),
            ]
    ...
    def test_builder_to_backend_stress(self, use_cpu_only, backend, rank, mean, dynamic):
        ...
        def build(x):
            return [
                mb.add(x=x, y=x),
                mb.random_normal(shape=shape, mean=mean, stddev=0.)
            ]

        def build_dyn(x, dyn_shape):
            return [
                mb.add(x=x, y=x),
                mb.random_normal(shape=dyn_shape, mean=mean, stddev=0.)
            ]
...
class TestRandomUniform:
    ...
    def test_builder_to_backend_smoke(self, use_cpu_only, backend):
        ...
        def build(x):
            return [
                mb.add(x=x, y=x),
                mb.random_uniform(shape=np.array([2, 1, 3], np.int32), low=0., high=0.),
                mb.random_uniform(shape=np.array([3, 1, 2], np.int32), low=1., high=1.),
            ]
