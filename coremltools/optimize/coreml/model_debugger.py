...
class ModelDebugger:
    ...
    def get_intermediate_output_names(
        self, op_include_fn=(lambda op: not (op.spec.type == "const"))  # op.spec.type == "cast" or
    ):
    ...
    def get_model_with_intermediate_outputs(
        ...
        # print(f"Creating model with intermediate outputs={intermediate_output_names}")
        model_key = frozenset(intermediate_output_names)
        model = self.__cached_models.get(model_key)
        if model is not None:
            # print(f"Found cached model with intermediate outputs={intermediate_output_names}")
            return model
    ...
    def step(
        ...
        for output_names in self.__class__.batch(intermediate_output_names, batch_size):
            # for output_names in intermediate_output_names:
