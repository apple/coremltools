...
class TestPyTorchConverterExamples:
    ...
    def test_convert_torch_vision_mobilenet_v2(tmpdir):
        ...
        np.testing.assert_allclose(results['1651'], expected.detach().numpy(),
                rtol=1e-2)
    ...
    def test_int64_inputs():
        ...
        result = mlmodel.predict({"input":
            example_input.detach().numpy().astype(np.float32)})
        ...
        with pytest.raises(ValueError, match=r"Duplicated inputs"):
            mlmodel = ct.convert(
                traced_model,
                inputs=[
                    ct.TensorType(name="input", shape=example_input.shape,
                    dtype=example_input.numpy().dtype),
                    ct.TensorType(name="input", shape=example_input.shape,
                    dtype=example_input.numpy().dtype),
                    ])
...
class TestMILExamples:
    ...
    def test_tutorial():
        ...
        @mb.program(input_specs=[
            mb.TensorSpec(shape=(1, 100, 100, 3)),
            ])
        def prog(x):
