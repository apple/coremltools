...
parser.add_argument('--set_version_stable', action='store_true', help='Set this version as the stable (main) version.')
...
def sanitize_names(path):
	if (os.path.isdir(path)):
		...
		for dir in onlydirs: sanitize_names(os.path.join(path, dir))
...
dirs = [os.path.join(docspath, f) for f in os.listdir(docspath) if os.path.isdir(os.path.join(docspath, f))]
for thisdir in dirs:
